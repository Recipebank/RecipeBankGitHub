...
public class ShoppingListClient {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < jArray.length(); i++) {
			...
			if (rId!=id) {
				System.out.println(jObject.get("RecipeTitle")+" "+jObject.get("RecipeId"));
				System.out.println("-----------"+jObject.get("ShoppingIngredientsId").toString()+","+jObject.get("IngredientName").toString()+","+jObject.get("IngredientMeasure").toString()+","+jObject.get("IngredientQuanlity").toString()+","+jObject.get("ShoppingIngredientState").toString());
				rId=id;
			}
			else {
				System.out.println("-----------"+jObject.get("ShoppingIngredientsId").toString()+","+jObject.get("IngredientName").toString()+","+jObject.get("IngredientMeasure").toString()+","+jObject.get("IngredientQuanlity").toString()+","+jObject.get("ShoppingIngredientState").toString());
